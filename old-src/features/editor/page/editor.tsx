...
const EditorPage: React.FC = () => {
  ...
  const { addSubtitle, setSelectedSubtitle, replaceSubtitle } = useSubtitles()
      ...
      try {
        ...
        if (data.data.subtitle !== null) {
          ...
          if (data.data != null) {
            ...
            replaceSubtitle(formattedSubtitles)
  ...
  const onGenerateWithAi = async () => {
    try {
      ...
      const response = await fetch(`${APIRoute.generateWithAI}/${project?.projectId}`);
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
  ...
  return (
    <div className="flex flex-col h-screen w-screen overflow-hidden bg-primary">
      ...
      <Headers projectName={project.projectName} showExport={() => setShowExport(true)} onGenerateWithAi={onGenerateWithAi} />
